...
def save_fig(path: Path) -> None:
    ...
    for ax in fig.axes:
        try:
            ...
            ax.tick_params(axis="both", which="major", colors="black", labelsize=13, width=1.0, length=6, direction="out", bottom=True, left=True, top=False, right=False)
            ax.tick_params(axis="both", which="minor", colors="black", width=0.8, length=3, direction="out", bottom=True, left=True, top=False, right=False)
...
def main() -> None:
    ...
    # Final RF refit on all literature-based training data for independent external evaluation
