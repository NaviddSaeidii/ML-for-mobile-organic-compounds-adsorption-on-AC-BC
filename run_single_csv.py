...
def engineer_features(df: pd.DataFrame, target: str) -> tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    ...
    df["has_aromatic_ring"] = (pd.to_numeric(df["number of aromatic rings"], errors="coerce") >= 1).astype(int)
    df["has_two_aromatics"] = (pd.to_numeric(df["number of aromatic rings"], errors="coerce") >= 2).astype(int)

    pos_vals = pd.to_numeric(df[pos_col], errors="coerce")
    neg_vals = pd.to_numeric(df[neg_col], errors="coerce")

    df["charge_state_anion"] = ((pos_vals == 0) & (neg_vals == 1)).astype(int)
    df["charge_state_cation"] = ((pos_vals == 1) & (neg_vals == 0)).astype(int)
    df["charge_state_neutral"] = ((pos_vals == 0) & (neg_vals == 0)).astype(int)
    df["charge_state_zwitterionic"] = ((pos_vals == 1) & (neg_vals == 1)).astype(int)
...
def complete_case_filter(X: pd.DataFrame, y: pd.Series, target: str) -> tuple[pd.DataFrame, pd.Series]:
    ...
    return merged.drop(columns=[target]), pd.to_numeric(merged[target], errors="coerce")
...
def attach_metadata(df_processed: pd.DataFrame, idx: pd.Index) -> pd.DataFrame:
    ...
    meta["Aromaticity"] = np.where(
        pd.to_numeric(meta["number of aromatic rings"], errors="coerce") >= 1,
        "Aromatic ring",
        "No ring",
    )
    return meta
...
def plot_si_correlations(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    for i, feat in enumerate(molecular_features):
        x = safe_series(df, feat)
        y = safe_series(df, "log Kd (L/kg)")
        valid = x.notna() & y.notna()
        axes[i].scatter(x[valid], y[valid], s=24, edgecolor="black", linewidth=0.3)
    ...
    pzc_feat = "PZC" if "PZC" in df.columns else "delta_PZCpH"
    adsorbent_features = ["surface area (m2/g)", "O wt%", pzc_feat]
    ...
    for ax, feat in zip(axes, adsorbent_features):
        ...
        y = safe_series(df, "log Kd (L/kg)")
        valid = x.notna() & y.notna()
        ax.scatter(x[valid], y[valid], s=24, edgecolor="black", linewidth=0.3)
        ax.set_xlabel(feat)
        ax.set_ylabel("log Kd (L/kg)")
        ax.set_title(feat)
    save_fig(out_dir / "Fig_2S_logKd_vs_adsorbent_properties.png")

    x = safe_series(df, "estimated log KOC  (L/kg)")
    y = safe_series(df, "log Kd (L/kg)")
    valid = x.notna() & y.notna()
    plt.figure(figsize=(5.1, 4.2))
    plt.scatter(x[valid], y[valid], s=24, edgecolor="black", linewidth=0.3)
    ...
    pairs = [
        ...
        ("surface area (m2/g)", pzc_feat),
        (pzc_feat, "O wt%"),
    ]
    fig, axes = plt.subplots(1, 3, figsize=(12, 3.8))
    for ax, (xcol, ycol) in zip(axes, pairs):
        x = safe_series(df, xcol)
        y = safe_series(df, ycol)
        valid = x.notna() & y.notna()
        ax.scatter(x[valid], y[valid], s=24, edgecolor="black", linewidth=0.3)
        ax.set_xlabel(xcol)
        ax.set_ylabel(ycol)
...
def evaluate_models_for_feature_set(X: pd.DataFrame, y: pd.Series, cfg: Config) -> dict:
    ...
    for name, estimator in MODEL_SPECS.items():
        model, scaler, X_train_s, X_test_s, y_pred = fit_pipeline_and_predict(
            estimator, X_train, X_test, y_train
        )
...
def load_external_evaluation_dataset(cfg: Config, training_n_rows: int, base_dir: Path) -> pd.DataFrame:
    combined = load_csv(base_dir / cfg.combined_csv)
    eval_df = combined.iloc[training_n_rows:].copy()

    if "delta_PZCpH" not in eval_df.columns and "PZC" in eval_df.columns:
        eval_df["delta_PZCpH"] = safe_series(eval_df, "PZC") - cfg.eval_pH_for_delta
...
def prepare_external_features(eval_df: pd.DataFrame, train_columns: list[str], target: str):
    ...
    available = [c for c in train_columns if c in X_eval_all.columns]
    X_eval = X_eval_all[available].copy()

    merged = (
        pd.concat([X_eval, y_eval.rename(target)], axis=1)
        .replace([np.inf, -np.inf], np.nan)
        .dropna(axis=0, how="any")
    )

    if merged.empty:
        print("No usable external-evaluation rows remained after filtering.")
        return None, None, None

    X_eval = merged.drop(columns=[target]).copy()
    y_eval = pd.to_numeric(merged[target], errors="coerce")

    missing = [c for c in train_columns if c not in X_eval.columns]
    if missing:
        print("External evaluation is missing required columns:")
        print(missing)
        return None, None, None
...
def plot_main_figure_8(model, scaler, X_eval, y_eval, eval_meta, out_dir: Path):
    ...
    eval_res["Aromaticity"] = np.where(
        pd.to_numeric(eval_meta["number of aromatic rings"], errors="coerce") >= 1,
        "Aromatic ring",
        "No ring",
    )
...
def main() -> None:
    ...
    combined_df = load_csv(base_dir / cfg.combined_csv)
    df_raw = combined_df.iloc[:cfg.train_rows].copy()
    ...
    if X_eval is not None and y_eval is not None and eval_processed is not None:
        eval_meta = attach_metadata(eval_processed, X_eval.index)

        final_scaler = StandardScaler()
        X_full_s_all = pd.DataFrame(
            final_scaler.fit_transform(X_full),
            index=X_full.index,
        ...
        final_rf.fit(X_full_s_all, y_full)

        eval_res = plot_main_figure_8(final_rf, final_scaler, X_eval, y_eval, eval_meta, out_dir)
        eval_res.to_csv(out_dir / "independent_evaluation_results.csv", index=True)
    else:
        print("Independent external evaluation skipped because no valid external rows were available.")
