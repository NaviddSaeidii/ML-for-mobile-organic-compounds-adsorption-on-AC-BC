...
class Config:
    ...
    repo_zip: str = "ML-for-mobile-organic-compounds-adsorption-on-AC-BC-main(2).zip"
...
def ensure_si_excel(base_dir: Path, cfg: Config) -> Path:
    ...
    if si_path.exists():
        return si_path
    zip_path = base_dir / cfg.repo_zip
    if not zip_path.exists():
        raise FileNotFoundError(
            f"Could not find '{cfg.si_excel}' or '{cfg.repo_zip}'. Put one of them next to the script."
        )
    with zipfile.ZipFile(zip_path) as zf:
        target_name = None
        for name in zf.namelist():
            if name.endswith(cfg.si_excel):
                target_name = name
                break
        if target_name is None:
            raise FileNotFoundError(f"'{cfg.si_excel}' was not found inside '{cfg.repo_zip}'.")
